...
script1_path = os.path.join(os.environ['EIC_PROJECT_DIR'], 'scripts', 'edit_XML_with_zpos.sh')
script2_path = os.path.join(os.environ['EIC_PROJECT_DIR'], 'scripts', 'call_npsim_eicrecon_analysis.sh')
...
with open(os.path.join(os.environ['EIC_PROJECT_DIR'], 'B0output', outputfile), 'r') as file:
    lines = file.readlines()
    last_line = lines[-1].strip()

# Process the last line
f1_str, f2_str = last_line.split()  # Correctly split the last line into two parts
f1 = float(f1_str)  # Convert the first part to float
